          ...
          children: <Widget>[
            ...
            Consumer<PaymentMethodModel>(builder: (context, model, child) {
              ...
              if (selectedId == null && model.paymentMethods.length > 0) {
                selectedId = model.paymentMethods.firstWhere((item) => item.enabled).id;
              }
                              ...
                              InkWell(
                                ...
                                child: Container(
                                  decoration: BoxDecoration(
                                      color: model.paymentMethods[i].id == selectedId
                                      ...
                                      children: <Widget>[
                                        ...
                                        Expanded(
                                          child: Column(
                                            crossAxisAlignment: CrossAxisAlignment.start,
                                            children: <Widget>[
                                              if (Payments[model.paymentMethods[i].id] != null)
                                                Image.asset(
                                                  Payments[model.paymentMethods[i].id],
                                                  width: 120,
                                                  height: 30,
                                                ),
                                              if (Payments[model.paymentMethods[i].id] == null)
                  ...
                  child: RaisedButton(
                    onPressed: () {
                      if (paymentMethodModel.paymentMethods.length > 0) {
                        ...
                        Provider.of<CartModel>(context).setPaymentMethod(paymentMethod);
                        final appModel = Provider.of<AppModel>(context);
                        if (paymentMethod.id == "cod" ||
                            appModel.appConfig["server"]["type"] != "woo") {
                          createOrder();
                        } else {
                          final user = Provider.of<UserModel>(context).user;
                          var params = Order().toJson(cartModel, user.id);
